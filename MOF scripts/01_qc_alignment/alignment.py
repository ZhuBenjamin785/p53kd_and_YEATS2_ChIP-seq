...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Align trimmed MOF paired-end reads to human and dm6."
    )
    parser.add_argument(
        "--check-only",
        action="store_true",
...
def main():
    args = parse_args()
    inputdir = os.environ.get("MOF_TRIMMED_DIR", os.path.join(project_dir, "MOF_trimmed"))
    bamfiles = os.environ.get("MOF_BAM_DIR", os.path.join(project_dir, "MOF_bamfiles"))
    genomes = [
        (
            os.environ.get(
                "HUMAN_BOWTIE2_INDEX",
                os.path.join(project_dir, "index", "human", "hgenome_index"),
            ),
            "human",
        ),
        (
            os.environ.get(
                "DM6_BOWTIE2_INDEX",
                os.path.join(project_dir, "index", "dm6", "dm6genome_index"),
            ),
            "dm6",
        ),
    ]
