...
peak_files <- c(
  Scr_MOF_consensus = "mof_macs3_results/consensus_peaks/Scr_MOF_consensus.bed",
  p53sh_MOF_consensus = "mof_macs3_results/consensus_peaks/p53sh_MOF_consensus.bed"
)

output_dir <- "mof_macs3_results/chipseeker_visualizations"
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

create_visualizations <- function(peak_files, file_prefix = "") {
  bad_files <- peak_files[!file.exists(peak_files) | file.info(peak_files)$size == 0]
  if (length(bad_files)) {
    stop("Missing or empty peak files:\n", paste(bad_files, collapse = "\n"))
  }
  ...
  for (sample_name in names(gene_ids)) {
    ...
    write.table(
      data.frame(geneId = gene_ids[[sample_name]]),
      file.path(output_dir, paste0(file_prefix, safe_name, "_gene_ids.txt")),
      sep = "\t",
      quote = FALSE,
      row.names = FALSE
    )
  ...
  summary_file <- file.path(output_dir, paste0(file_prefix, "annotation_summary.tsv"))
  ...
  combined_pdf <- file.path(
    output_dir,
    paste0(file_prefix, "chipseeker_annotation_combined.pdf")
  )
  ggsave(
  ...
  pdf_file <- file.path(
    output_dir,
    paste0(file_prefix, "chipseeker_annotation_plots.pdf")
  )
  ...
  output_files <- c(pdf_file, combined_pdf, summary_file)
...
create_visualizations(peak_files)
