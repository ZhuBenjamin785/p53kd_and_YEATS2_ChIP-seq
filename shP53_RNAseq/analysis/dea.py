...
default_counts = project_dir / "shared" / "rna_seq_featurecounts" / "rna_seq_featureCounts_cleaned.txt"
default_metadata = project_dir / "rna_seq_metadata.tsv"
default_output = project_dir / "shared" / "rna_seq_dea"
...
def run_dea(counts_path, metadata_path, output_dir, case=None, control=None, min_cpm=0.3, min_samples=2):
    ...
    if (replicate_counts < 2).any():
        insufficient = ", ".join(
            f"{condition} ({count} sample)"
            for condition, count in replicate_counts.items()
            if count < 2
        )
        raise SystemExit(
            "DESeq2 requires at least two biological replicates per condition; "
            f"the current metadata has insufficient replication: {insufficient}. "
            "Use rna_dea.sh for exploratory fold changes or add biological replicates."
        )
