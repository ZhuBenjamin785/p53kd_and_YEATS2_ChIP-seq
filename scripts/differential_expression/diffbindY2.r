...
results_dir <- "diffbind_results_yeats2"
...
samples <- data.frame(
  SampleID = c("Scramble_rep1", "Scramble_rep2", "YEATS2KD_rep1", "YEATS2KD_rep2"),
  Condition = c("Scramble", "Scramble", "YEATS2KD", "YEATS2KD"),
  Replicate = c(1, 2, 1, 2),
  bamReads = c(
  ...
  Peaks = c(
    "macs3_results_yeats2/peaks/Scramble_H4K16ac_rep1/Scramble_H4K16ac_rep1_peaks.broadPeak",
    "macs3_results_yeats2/peaks/Scramble_H4K16ac_rep2/Scramble_H4K16ac_rep2_peaks.broadPeak",
    "macs3_results_yeats2/peaks/YEATS2KD_H4K16ac_rep1/YEATS2KD_H4K16ac_rep1_peaks.broadPeak",
    "macs3_results_yeats2/peaks/YEATS2KD_H4K16ac_rep2/YEATS2KD_H4K16ac_rep2_peaks.broadPeak"
  ),
  PeakCaller = "macs",
...
write.csv(samples, file.path(results_dir, "diffbind_yeats2_scramble_samples.csv"), row.names=FALSE)
...
pdf(file.path(results_dir, "DiffBind_YEATS2_PCA.pdf"))
dba.plotPCA(db, attributes=DBA_CONDITION)
dev.off()

pdf(file.path(results_dir, "DiffBind_YEATS2_correlation_heatmap.pdf"))
plot(db)
dev.off()
...
write.csv(
  all_df,
  file.path(results_dir, "DiffBind_YEATS2_all_peaks.csv"),
  row.names=FALSE
)
...
write.csv(
  sig,
  file.path(results_dir, "DiffBind_YEATS2_significant_peaks.csv"),
  row.names=FALSE
)
...
write.csv(gained, file.path(results_dir, "YEATS2_H4K16ac_gained_peaks.csv"), row.names=FALSE)
write.csv(lost, file.path(results_dir, "YEATS2_H4K16ac_lost_peaks.csv"), row.names=FALSE)
...
write.csv(
  ...
  file.path(results_dir, "YEATS2_H4K16ac_gained_annotated.csv"),
  row.names=FALSE
)

write.csv(
  as.data.frame(loss_anno),
  file.path(results_dir, "YEATS2_H4K16ac_lost_annotated.csv"),
  row.names=FALSE
)

pdf(file.path(results_dir, "YEATS2_gain_peak_annotation.pdf"))
plotAnnoPie(gain_anno)
dev.off()

pdf(file.path(results_dir, "YEATS2_loss_peak_annotation.pdf"))
...
if (requireNamespace("clusterProfiler", quietly=TRUE) &&
  ...
  write.csv(as.data.frame(gain_go), file.path(results_dir, "YEATS2_H4K16ac_gain_GO_BP.csv"), row.names=FALSE)
  write.csv(as.data.frame(loss_go), file.path(results_dir, "YEATS2_H4K16ac_loss_GO_BP.csv"), row.names=FALSE)

  pdf(file.path(results_dir, "YEATS2_H4K16ac_gain_GO.pdf"))
  if (nrow(as.data.frame(gain_go)) > 0) {
    print(dotplot(gain_go, showCategory=15) +
      ggtitle("Processes associated with gained H4K16ac after YEATS2 KD"))
  } else {
    plot.new()
    text(0.5, 0.5, "No significant GO terms for gained H4K16ac after YEATS2 KD")
  }
  dev.off()

  pdf(file.path(results_dir, "YEATS2_H4K16ac_loss_GO.pdf"))
  if (nrow(as.data.frame(loss_go)) > 0) {
    print(dotplot(loss_go, showCategory=15) +
      ggtitle("Processes associated with lost H4K16ac after YEATS2 KD"))
  } else {
    plot.new()
    text(0.5, 0.5, "No significant GO terms for lost H4K16ac after YEATS2 KD")
  }
  dev.off()
} else {
