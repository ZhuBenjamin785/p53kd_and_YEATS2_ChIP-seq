...
peak_files <- c(
  p53_0hr = "fastqchip_macs3_results/peaks/p53_0hr/p53_0hr_peaks.narrowPeak"
)
...
output_dir <- "fastqchip_macs3_results/chipseeker_visualizations_p53_0hr"
...
names(annotations) <- names(peak_files)
...
write_genes <- function(ids, filename) {
  write.table(
    data.frame(geneId = ids), filename,
...
summary_table <- data.frame(
  ...
  annotated_peak_count = vapply(
    annotations, function(x) nrow(as.data.frame(x)), integer(1)
  ),
  unique_gene_count = vapply(gene_ids, length, integer(1))
)
write.table(
  summary_table, file.path(output_dir, "annotation_summary.tsv"),
  sep = "\t", quote = FALSE, row.names = FALSE
)
...
output_files <- c(
  ...
  file.path(output_dir, "p53_0hr_gene_ids.txt")
)
...
message("Wrote ChIPseeker visualizations to: ", output_dir)
